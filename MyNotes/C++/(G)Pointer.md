...
**引用参数：** 
C++中引入引用的主要目的是将引用作为函数的参数,非const的情况调用时对应的实参必须是左值表达式
```cpp
#指针参数
...
x = 5  
```
引用传递是地址传递的另一种更简单明了的实现方法。在C++程序中，函数参数经常采用引用传递。利用引用传递的好处是减少函数调用时的开销：不需要分配新的内存空间；不需要复制实际参数的数值。如果在函数内不许改变参数的值，则形式参数用const限定，此时实参数可以绑定到右值（比如表达式）。对非const的形式参数采用引用传递时，实际参数不能是常量或临时量。注意判断什么时候会创建临时对象
 ```cpp
...
**返回引用的函数**：函数的返回值可以是一个引用。它表示函数的返回值是函数内某一个变量（不能是局部变量）的引用，这个时候不会再创建一个临时对象。返回引用的函数原型：`类型 &函数名 (形式参数表)`
```cpp
//交换a,b,c中的最大值和最小值
int &max(int &a, int &b, int &c) { //返回最大的变量的地址
    if (a>b)
...
**右值引用**
左值表达式是指运算结果有固定的内存空间，右值表达式是指运算结束后就不再存在的临时变量。区分的便捷方式是看能不能对表达式取地址。
 右值引用可以延长临时对象的生命周期，保留了临时对象的空间和值。
```cpp
int x = 10
int &&y = x+9;
int &&z = 9+8;
...
// 比较函数：递增排序（前一个大于后一个时返回 true，需要交换）
bool increaseInt(int a, int b)
{
    return a > b;
}

// 比较函数：递减排序（前一个小于后一个时返回 true，需要交换）
bool decreaseInt(int a, int b)
{
    return a < b;
}
