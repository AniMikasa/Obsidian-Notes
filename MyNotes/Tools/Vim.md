...
- `d{motion}` delete {motion}
    - e.g. `dw` is delete word, `d$` is delete to end of line, `d0` is delete to beginning of line,`dd`delete the whole line.
- `c{motion}` change {motion}
    - e.g. `cw` is change word, `cc` is change the whole line.
...
- `y` to copy / “yank” (some other commands like `d` also copy),like `yy` `yw`
- `p` to paste
